...
async function boot() {
  ...
  async function doLoadFile(file, preserveStems = false) {
    ...
    try {
      ...
      currentFile = file
      document.getElementById('session-file-status').textContent = `本頁保留：${file.name}。一般母帶處理未上傳音檔。`
  ...
  // Clear the audio owners together; retain DSP settings for the next track.
  function clearSession() {
    if (isLoadingFile || stemController.busy || document.getElementById('processing-overlay').classList.contains('visible') || document.getElementById('final-preview-btn').disabled) {
      setStatus('音訊仍在處理中，請待處理結束後清除。', false)
      return false
    }
    engine.cancelPendingLoad()
    engine.stop()
    ...
    document.querySelectorAll('.analysis-value').forEach(value => { value.textContent = '—' })
    document.querySelectorAll('.analysis-conf-fill').forEach(fill => { fill.style.width = '0%' })
    if (progressFill) progressFill.style.width = '0%'
    progressEl?.setAttribute('aria-valuenow', '0')
    stemController.clear()
    album.clear(); renderAlbum()
    loudnessHistory.clear()
    updatePlayBtn(false)
    document.querySelectorAll('input[type="file"]').forEach(input => { input.value = '' })
    document.getElementById('upload-text-wrap').textContent = '選取音檔 或拖拉音檔'
    document.getElementById('waveform-main').setAttribute('aria-label', '主波形：尚未載入音檔')
    for (const id of ['analyze-btn', 'export-btn', 'album-add-btn', 'trim-toggle', 'trim-clear', 'zoom-in', 'zoom-out', 'zoom-fit']) document.getElementById(id).disabled = true
    document.getElementById('trim-toggle').classList.remove('active')
    document.getElementById('trim-clear').hidden = true
    for (const id of ['time-current', 'time-total']) document.getElementById(id).textContent = '0:00'
    document.getElementById('session-file-status').textContent = '已清除此頁的來源、分軌、專輯與預覽。已下載的檔案仍保留在你的裝置。'
    document.getElementById('final-preview-status').textContent = '音訊已清除，請重新選取音檔。'
    setStatus('已清除此頁音訊，可以開始下一首。', true)
    setDot('')
    if (import.meta.env.DEV) window.__wf = { engine, album }
    return true
  }
  ...
  fileInput.addEventListener('change', async () => {
    ...
    if (files.length === 1) {
      ...
      for (const f of files) {
        album.add({ title: f.name.replace(/\.[^.]+$/, ''), file: f, snapshot })
      }
      await loadFile(files[0])
      renderAlbum()
  ...
  document.getElementById('export-btn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      document.getElementById('session-file-status').textContent = '已輸出母帶。音檔仍在本頁供編輯；完成後可按「清除音檔、分軌與專輯」。'
      if (document.getElementById('clear-after-export').checked) clearSession()
    } catch (err) {
      setProcessing(false, '', 0)
